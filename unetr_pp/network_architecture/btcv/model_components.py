...
class ux_block(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6):
        ...
        # self.dwconv = nn.Conv3d(dim, dim, kernel_size=5, padding=2, groups=dim) #82.81
        # self.dwconv = nn.Conv3d(dim, dim, kernel_size=9, padding=4, groups=dim)
        # self.dwconv = nn.Conv3d(dim, dim, kernel_size=11, padding=5, groups=dim)
...
class UnetrPPEncoder(nn.Module):
    def __init__(self, input_size=[32 * 32 * 32, 16 * 16 * 16, 8 * 8 * 8, 4 * 4 * 4],dims=[32, 64, 128, 256],
                 proj_size =[64,64,64,32], depths=[3, 3, 3, 3],  num_heads=4, spatial_dims=3, in_channels=1, dropout=0.0, transformer_dropout_rate=0.15 ,drop_path_rate=0., layer_scale_init_value=1e-6, out_indices=[0, 1, 2, 3],**kwargs):
        super().__init__()
    ...
    def forward_features(self, x):
        #x: [1, 64, 128, 128] [32, 32, 32, 32] [64, 16, 16, 16] [128, 8, 8, 8] [256, 4, 4, 4]
        outs = []
        for i in range(4):
