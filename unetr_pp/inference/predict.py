...
def preprocess_multithreaded(trainer, list_of_lists, output_files, num_processes=2, segs_from_prev_stage=None):
    ...
    assert isinstance(trainer, Trainer_acdc) or isinstance(trainer, Trainer_synapse) or isinstance(trainer, Trainer_tumor) or isinstance(trainer, Trainer_lung) or isinstance(trainer, Trainer_btcv)
    q = Queue(1)
    processes = []
...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
    ...
    :return:
    """
    maybe_mkdir_p(output_folder) #生成文件夹
